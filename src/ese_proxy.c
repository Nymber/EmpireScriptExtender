...
/* Lua API calls made by ESE can themselves pass through our detours. Suppress
 * observer/pump work while ESE registers natives or executes autoexec;
 * recursively pumping Lua inside Lua string/table operations corrupted the
 * active state during battle startup (observed 2026-09-23). */
static volatile LONG g_lua_hook_suppressed = 0;
static int lua_hooks_suppressed(void) {
    return InterlockedCompareExchange(&g_lua_hook_suppressed, 0, 0) != 0;
}
...
static void register_natives(lua_State* L) {
    /* setfield itself is hooked. Ignore the nested notifications generated by
     * our own registrations; they must not pump commands or re-enter Lua. */
    InterlockedIncrement(&g_lua_hook_suppressed);
    for (int i = 0; kNatives[i].name; i++) {
        L_.pushcclosure(L, kNatives[i].fn, 0);
        L_.setfield(L, LUA_GLOBALSINDEX, kNatives[i].name);
    }
    InterlockedDecrement(&g_lua_hook_suppressed);
...
static void __cdecl on_setfield(DWORD* stk) {
    if (lua_hooks_suppressed()) return;
